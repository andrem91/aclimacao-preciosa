    ...
    <>
      ...
      <section className="relative isolate h-[590px] bg-[#264c3c] text-white 2xl:h-[640px]">
        <Image
          ...
          sizes="100vw"
        />
        <div className="absolute inset-0 bg-[linear-gradient(90deg,#082319bf,#08231940),linear-gradient(0deg,#0823199c,transparent_45%)] md:bg-[linear-gradient(90deg,rgba(8,34,25,0.76),rgba(8,34,25,0.25)_75%),linear-gradient(0deg,rgba(8,34,25,0.55),transparent_28%)]" />
        <div
          className={`${pageContainer} relative pt-[92px] md:pt-[94px] 2xl:pt-28`}
        >
          <p
            className={`${eyebrow} mb-[21px] flex items-center gap-2.5 tracking-[0.13em] text-[#e5e9cd] md:tracking-[0.16em] text-xs`}
          >
          ...
          <h1 className="max-w-[800px] text-[46px] leading-[1.18] font-[450] tracking-[-0.03em] md:text-[clamp(44px,5vw,65px)] md:leading-[1.16]">
            ...
            de um <em className="text-[#dce7c5]">novo jeito.</em>
          </h1>
          <p className="mt-6 mb-[30px] text-base leading-[1.9] text-[#f1f1e5] md:mt-[23px] md:text-lg">
        ...
        <div
          className={`${pageContainer} absolute inset-x-0 bottom-[21px] flex flex-wrap items-center justify-between gap-2 text-xs tracking-[0.06em] text-[#e0e4d9] md:bottom-6 md:flex-nowrap md:gap-2.5`}
        >
      ...
      <section className="py-[49px] md:py-[78px] border-y border-[#e2e4d9] bg-[#eaece3]">
        ...
        <div className="grid grid-cols-1 grid-rows-[350px_260px_260px] gap-[18px] md:grid-cols-[1.16fr_1fr] md:grid-rows-[230px_230px] md:gap-5">
          {places.map((p, i) => (
            <Link
              key={p.id}
              href={"/lugares/" + p.slug}
              className={`group relative overflow-hidden rounded bg-[#244635] text-white ${i === 0 ? "md:row-span-2" : ""}`}
            >
              <Image
